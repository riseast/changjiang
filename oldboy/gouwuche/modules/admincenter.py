...
def freezing(account):
    ...
    with open(db_path_user,'r',encoding='utf-8') as fh:
        ...
        fd = eval(fr)
        if fd['status'] == 0:
            print('\033[31;1m当前信用卡 【已冻结】\033[0m')
        if fd['status'] == 1:
            free = input('\033[33;1m当前信用卡 【未冻结】 按任意键选择冻结 按b返回>>>\033[0m')
            if free != 'b':
...
def defrosting(account):
    ...
    with open(db_path_user, 'r', encoding='utf-8') as fh:
        ...
        fd = eval(fr)
        if fd['status'] == 1:
            print('\033[31;1m当前信用卡 【未冻结】\033[0m')
        if fd['status'] == 0:
            free = input('\033[33;1m当前信用卡 【已冻结】 按任意键选择解冻 按b返回>>>\033[0m')
            if free != 'b':
                with open(db_path_user, 'w', encoding='utf-8') as fw:
                    res = fr.replace(str(fd["status"]), '1', 1)
                    fw.write(res)
                    print('\033[31;1m当前信用卡 【已解冻】\033[0m')
